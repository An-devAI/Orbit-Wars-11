...
def phase7_rear_push(w: WorldModel, spent: dict, moves: list, targeted: set, planned_commitments: list):
    ...
    for src in w.my_planets:
        bud = w.attack_budget(src.id, spent) - 5
        if bud < REAR_MIN_SHIPS: continue
        
        # Đặc biệt cho sao chổi sắp hết hạn: rút quân khẩn cấp
        evacuating = False
        if src.id in w.comet_ids:
            life = comet_life(src.id, w.comet_map)
            if life < COMET_EVAC_TURNS:
                bud = int(src.ships) - spent.get(src.id, 0) # Rút sạch quân
                if bud < 1: continue
                evacuating = True

        send = int(bud * REAR_SEND_RATIO) if not evacuating else bud
        if send < 1: continue
        ...
        tag = "(relay)" if not evacuating else "(EVAC)"
        print(f"[PUSH] Turn {w.step}: {src.id} -> {relay.id} {tag} | angle={angle:.3f}, ships={send}, turns={turns}")
