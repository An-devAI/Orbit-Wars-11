...
class WorldModel:
    ...
    def aim(self, src, tgt, ships):
        ...
        # Optimization removed because target planet can move towards the source
        start_t = 1

        for t in range(start_t, 150):
            # Dùng t thay vì t - 1 vì sau t turns tàu đến nơi, hành tinh cũng ở vị trí của t turns sau
        ...
        # Kiểm tra xem góc né có quá lệch so với hành tinh mục tiêu không
        ...
        # Siết margin (Nới lỏng từ 0.3/0.45 lên 0.6/0.8 để tránh bỏ lỡ các phát bắn khả thi)
        margin = 0.6 if tgt.id in self.comet_ids else 0.8
        if angle_diff > target_angular_width * margin:
            self._shot_cache[key] = None
            return None

        # KIỂM TRA CHỐT: Thử mô phỏng tia bắn thực tế xem có chạm đích không
        # Nếu tia bắn sau khi né Mặt Trời không chạm vào vòng tròn hành tinh ĐÍCH, ta bỏ qua
        if not path_hits_planet(src.x, src.y, src.x + math.cos(safe_ang)*1000, src.y + math.sin(safe_ang)*1000, tx, ty, tgt.radius * 0.95):
            self._shot_cache[key] = None
            return None
...
def build_missions(w: WorldModel, spent: dict, targeted: set, planned_commitments: list, phase: str) -> list[Mission]:
        ...
        for tgt in pool:
            ...
            if wait_turns > 0:
                ...
                for relay in w.my_planets:
                    ...
                    # Điều kiện 1: Relay phải "tiến gần" về phía Tgt hơn
    ...
    for src in w.my_planets:
        ...
        if allow_rear_push and budget >= REAR_MIN_SHIPS and dist_to_front:
            ...
            # Tìm hành tinh đồng minh ở "phía trước" mình (gần frontier hơn)
