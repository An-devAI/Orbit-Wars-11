...
def position_bonus(w: WorldModel, tgt: Planet, phase: str) -> float:
    ...
    if phase == "expand":
        if tgt.owner == -1: bonus *= 2.0  # Ưu tiên chiếm mỏ trung lập giai đoạn đầu
    else: # combat
        if tgt.owner not in (-1, w.player): bonus *= 1.5 # Ưu tiên đánh chiếm hành tinh địch
...
def build_missions(w: WorldModel, spent: dict, targeted: set, planned_commitments: list, phase: str) -> list[Mission]:
    ...
    for src in w.my_planets:
            ...
            if mtype != "DEFEND":
                for arr_eta, arr_o, arr_s in w.arrivals.get(tgt.id, []):
                    if arr_o not in (-1, w.player):
                        threat_sum += arr_s * (total_turns / max(total_turns, float(arr_eta)))**2
                for en in w.enemy_planets:
                    eta_en = math.ceil(dist(en.x, en.y, tgt.x, tgt.y) / fleet_speed(en.ships))
                    threat_sum += en.ships * (total_turns / max(total_turns, float(eta_en)))**2
            threat_total = min(1.5, threat_sum / max(1.0, float(w.my_total)))
        ...
        # Rear Push logic (giữ nguyên hoặc tinh chỉnh sau)
        if allow_rear_push and (int(src.ships) - spent.get(src.id, 0) - keep) >= REAR_MIN_SHIPS and dist_to_front:
            budget = int(src.ships) - spent.get(src.id, 0) - keep
            send = int(budget * REAR_SEND_RATIO)
            candidates = [p for p in w.my_planets if dist_to_front.get(p.id, 0) < dist_to_front.get(src.id, 0) - 5]
            if candidates:
                ...
                if aim_r:
                    ...
                    if turns <= REAR_MAX_TURNS and is_path_safe(w, src.id, src.x, src.y, angle, send, relay.id):
                        ...
                        raw_missions.append((src.id, relay.id, angle, turns, send, 0.0, position_bonus(w, relay, phase), ships_at_arrival_r, 0.0, "RELAY", 0))
...
def update_params(step, initial_by_id, planets):
    ...
    if is_1v1:
        if is_early:
            # --- SETUP 1: 1vs1, Đầu game (Mở rộng nhanh) ---
            ALPHA = 1.3             # Ưu tiên chiếm mỏ cực cao
            BETA = 0.20             # Sẵn sàng chi lính để lấy mỏ to sớm
            GAMMA = 0.70            
            DELTA = 0.8             
            EPSILON = 1.0
            MAX_ARRIVAL_TURNS = 25  
            PROACTIVE_RATIO = 0.15  # Tăng nhẹ để tránh bị mất mỏ vào tay địch
            REAR_SEND_RATIO = 0.80  
            MIN_ATTACK_SHIPS = 3
        else:
            # --- SETUP 2: 1vs1, Cuối game (Hủy diệt) ---
            ALPHA = 1.0
            BETA = 0.05             # Đánh đổi lính tối đa để dứt điểm
            GAMMA = 0.50            # Không ngại đi xa
            DELTA = 1.0            
            EPSILON = 1.0
            MAX_ARRIVAL_TURNS = 40  # Tầm đánh cực xa
            PROACTIVE_RATIO = 0.35  # Đề phòng địch phản công tuyệt vọng
            REAR_SEND_RATIO = 0.95  
            MIN_ATTACK_SHIPS = 3

    else: # CHẾ ĐỘ 1VS3 (FFA)
        if step <= 15:
            # --- SETUP 3: 1vs3, Đầu game (Phòng thủ & Ngăn chặn) ---
            # Mục tiêu: Vừa thủ góc, vừa phải cướp các mỏ trung lập mà Top 1 đang nhắm tới.
            ALPHA = 3.0             # Vẫn phải ưu tiên kinh tế để không bị thọt Prod
            BETA = 0.70             # (Giảm từ 0.6) Chấp nhận tốn lính hơn một chút để tranh giành với Top 1
            GAMMA = 1.50            # Cho phép bay xa hơn một chút để quấy rối
            DELTA = 1.0            # Giảm bớt sự "nhát" để dám ra ngoài tranh chấp
            EPSILON = 1.0
            MAX_ARRIVAL_TURNS = 20  
            PROACTIVE_RATIO = 0.40  
            REAR_SEND_RATIO = 0.50  
            MIN_ATTACK_SHIPS = 3
        else:
            # --- SETUP 4: 1vs3, Giữa/Cuối game (KIỀM CHẾ TOP 1) ---
            # Mục tiêu: "Tỉa" các hành tinh ít quân của Top 1 để kìm Prod của nó lại.
            ALPHA = 3.0             # Rất coi trọng việc cướp Prod (lấy của nó đập vào mình)
            BETA = 0.70             # (Giảm từ 0.85) Chấp nhận "Surplus" ít hơn khi đánh Top 1
            GAMMA = 1.20            # Tăng tầm bay để có thể thọc sâu vào nách Top 1
            DELTA = 1.8            # Tăng mạnh DELTA để dùng logic Bonus điều hướng mục tiêu
            EPSILON = 3.0
            MAX_ARRIVAL_TURNS = 25  
            PROACTIVE_RATIO = 0.10
            REAR_SEND_RATIO = 0.90  
            MIN_ATTACK_SHIPS = 3
