...
def build_missions(w: WorldModel, spent: dict, targeted: set, planned_commitments: list, phase: str) -> list[Mission]:
    ...
    m_p, s_p = get_stats([m[5] for m in raw_missions], 0.0, 20.0); m_t, s_t = get_stats([m[3] + m[10]*0.5 for m in raw_missions], 35.0, 15.0)
    ...
    for r in raw_missions:
        ...
        eff_t = trn + wait * 0.5; np = (prf - m_p) / s_p; nt = (eff_t - m_t) / s_t; nb = (bon - m_b) / s_b; nc = (abs(shp - cst) - m_c) / s_c; nth = (thrt - m_th) / s_th
