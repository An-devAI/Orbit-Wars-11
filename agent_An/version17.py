...
def time_penalty(t):
    ...
    """
    return t if t <= 10 else 10 + ((t - 10) ** 1.5)
    ...
    def aim(self, src, tgt, ships, max_wait=5):
        ...
        for wait in range(max_wait + 1):
            ...
            for t_float in [x * 0.5 for x in range(2, 300)]:
                ...
                d = dist(src_pos[0], src_pos[1], pos[0], pos[1])
                    ...
                    if t_arr < best_arrival:
                        ...
                        if angle_diff <= target_angular_width * 0.8:
                            ...
                            if seg_min_dist(src_pos[0], src_pos[1], ex, ey, tx, ty) <= tgt.radius + 0.1:
                                best_arrival = t_arr
                                best_res = (safe_ang, int(math.ceil(best_arrival)), tx, ty, wait)
                    break
...
def build_missions(w: WorldModel, spent: dict, targeted: set, planned_commitments: list, phase: str) -> list[Mission]:
    ...
    for src in w.my_planets:
        ...
        for tgt in pool:
            ...
            if mtype == "CAPTURE":
                ...
                # Anti-vulture & Reinforcement:
                # Calculate cost to hold for 10 turns after arrival.
                budget_at_fire = src_tl["ships_at"].get(wait_turns, src.ships) - spent.get(src.id, 0) - keep
                cost_to_hold = min_ships_to_own_at(w, tgt.id, total_turns, extra, sim_until=total_turns + 10)
                
                if cost_to_hold <= budget_at_fire:
                    # Upgrade req_cost to ensure we actually HOLD it.
                    req_cost = max(req_cost, cost_to_hold)
                else:
                    # If we can't afford to hold, check if there's an actual threat.
                    # Fleet already in flight arriving shortly after us?
                    has_vulture = any(a[1] not in (-1, w.player) and a[0] > total_turns and a[0] <= total_turns + 12 for a in w.arrivals.get(tgt.id, []))
                    if has_vulture:
                        continue
                    # Or maybe an enemy planet is so close they can vulture us easily?
                    # (This is more speculative, but let's stick to in-flight fleets for now as requested).
    ...
    m_p, s_p = get_stats([m[5] for m in raw_missions], 0.0, 20.0)
    m_t, s_t = get_stats([time_penalty(m[3]) for m in raw_missions], 35.0, 15.0)
    m_b, s_b = get_stats([m[6] for m in raw_missions], 0.0, 5.0)
    m_c, s_c = get_stats([(m[7] - m[4]) for m in raw_missions], 0.0, 15.0)
    m_th, s_th = get_stats([m[8] for m in raw_missions], 0.1, 0.3)
    missions = []
    for r in raw_missions:
        sid, tid, ang, trn, cst, prf, bon, shp, thrt, mtyp, wait = r[:11]; rfid, rlock = (r[11], r[12]) if len(r) > 11 else (None, None)
        pt = time_penalty(trn); np = (prf - m_p) / s_p; nt = (pt - m_t) / s_t; nb = (bon - m_b) / s_b; nc = (abs(shp - cst) - m_c) / s_c; nth = (thrt - m_th) / s_th
        score = (ALPHA * np + BETA * nc - GAMMA * nt + DELTA * nb - EPSILON * nth)
        debug = {"raw": {"profit": prf, "cost": cst, "turns": trn, "wait": wait, "eff_t": pt, "bonus": bon, "threat": thrt, "ships_at": shp}, "norm": {"p": np, "c_diff": nc, "t": nt, "b": nb, "th": nth}, "weights": {"A": ALPHA, "B": BETA, "G": GAMMA, "D": DELTA, "E": EPSILON}}
