...
def time_penalty(t):
    """
    Non-linear time penalty: linear up to 10 turns, then power of 1.5.
    Encourages closer targets significantly.
    """
    return t if t <= 5 else 5 + ((t - 5) ** 2.0)
...
def is_vulnerable_frontline(w, src_id, dist_threshold=40.0, strength_ratio=1.5):
    """
    Checks if a planet is on the frontline and significantly weaker than nearby enemies.
    Used to block risky expansions from critical defensive positions.
    """
    p_src = w.planet_by_id.get(src_id)
    if not p_src: return False
    src_pos = (p_src.x, p_src.y)
    enemy_nearby_ships = 0
    is_front = False
    for p in w.planets:
        if p.owner not in (-1, w.player):
            d = dist(src_pos[0], src_pos[1], p.x, p.y)
            if d < dist_threshold:
                is_front = True
                enemy_nearby_ships += p.ships
    if not is_front: return False
    return enemy_nearby_ships > p_src.ships * strength_ratio
...
class WorldModel:
    ...
    def aim(self, src, tgt, ships, max_wait=5):
        ...
        for wait in range(max_wait + 1):
            src_pos = self.predict_pos(src.id, wait)
            if src_pos is None: continue
            for t_flight in range(1, 150):
                t_arr = wait + t_flight
                pos = self.predict_pos(tgt.id, t_arr)
                if pos is None: break 
                tx, ty = pos[0], pos[1]
                direct_angle = math.atan2(ty - src_pos[1], tx - src_pos[0])
                safe_ang = safe_angle(src_pos[0], src_pos[1], tx, ty) 
                fx = src_pos[0] + math.cos(safe_ang) * spd * t_flight
                fy = src_pos[1] + math.sin(safe_ang) * spd * t_flight
                if dist(fx, fy, tx, ty) <= tgt.radius:
                    if t_arr < best_arrival:
                        angle_diff = abs((safe_ang - direct_angle + math.pi) % (2 * math.pi) - math.pi)
                        dist_to_tgt = dist(src_pos[0], src_pos[1], tx, ty)
                        target_angular_width = math.atan2(tgt.radius, max(1.0, dist_to_tgt))
                        if angle_diff <= target_angular_width * 0.8:
                            ex = src_pos[0] + math.cos(safe_ang) * 1000; ey = src_pos[1] + math.sin(safe_ang) * 1000
                            if seg_min_dist(src_pos[0], src_pos[1], ex, ey, tx, ty) <= tgt.radius + 0.1:
                                best_arrival = t_arr
                                best_res = (safe_ang, t_arr, tx, ty, wait)
                    break
        self._shot_cache[key] = best_res; return best_res
...
def build_missions(w: WorldModel, spent: dict, targeted: set, planned_commitments: list, phase: str) -> list[Mission]:
    ...
    pool = [p for p in w.planets if not (p.id in targeted or p.id in w.comet_ids or (p.owner == w.player and w.holds_full(p.id)))]
    ...
    for src in w.my_planets:
        ...
        for tgt in pool:
            ...
            aim_res = w.aim(src, tgt, max(1, int(src.ships)))
            if not aim_res: continue
            angle, total_turns, tx, ty, traj_wait = aim_res
            if total_turns > MAX_ARRIVAL_TURNS: continue
            
            flight_time = total_turns - traj_wait
            ...
            wait_turns = traj_wait
            if src_tl["ships_at"][0] < req_cost + keep:
                for dt in range(1, 21):
                    if src_tl["ships_at"].get(dt, 0) >= req_cost + keep:
                        wait_turns = max(wait_turns, dt)
                        break
            
            if wait_turns > 15: continue
            if wait_turns > traj_wait:
                ...
                aim_f = w.aim(src_f, tgt, req_cost, max_wait=0)
                if not aim_f: continue
                angle, flight_time, _, _, _ = aim_f
                total_turns = wait_turns + flight_time
                req_cost = min_ships_to_own_at(w, tgt.id, total_turns, extra)
                if req_cost == 0 or req_cost >= 9999: continue
            ...
            # Frontline defense constraint
            if mtype == "CAPTURE" and is_vulnerable_frontline(w, src.id): continue
            ...
            if mtype == "CAPTURE":
                ...
                if enemy_arr:
                    ...
                    wait_needed = (last_enemy_t + 1) - flight_time
                    my_budget = int(src.ships) - spent.get(src.id, 0) - keep
                    cap_cost = min_ships_to_own_at(w, tgt.id, total_turns, extra)
                    should_steal = False
                    if total_turns > last_enemy_t: should_steal = True
                    ...
                    if should_steal and wait_needed <= 15:
                        wait_turns = max(wait_turns, wait_needed)
                        total_turns = wait_turns + flight_time
                        req_cost = min_ships_to_own_at(w, tgt.id, total_turns, extra)
                        mtype = "STEAL"; bonus += 500.0
                        tl_tgt = w.timelines[tgt.id]
                        enemy_surv = tl_tgt["ships_at"].get(last_enemy_t, tl_tgt["ships_at"][tl_tgt["horizon"]])
            ...
            if mtype in ("ATTACK", "CAPTURE", "STEAL", "DEFEND"):
                ...
                if full_send >= req_cost:
                    ...
                    if aim_full:
                        fa, ft_res, _, _, fw = aim_full
                        if mtype == "STEAL":
                            if target_arrival_t >= ft_res:
                                if (target_arrival_t - ft_res) <= 15: 
                                    angle, total_turns, wait_turns, req_cost = fa, target_arrival_t, fw + (target_arrival_t - ft_res), full_send
                        elif wait_turns == 0 and ft_res <= MAX_ARRIVAL_TURNS + 5:
                            req_at_full = min_ships_to_own_at(w, tgt.id, ft_res, extra)
                            if (mtype == "DEFEND" and full_send > 0) or (0 < req_at_full <= full_send):
                                angle, total_turns, wait_turns, req_cost = fa, ft_res, fw, full_send
            ...
            if threat_total > 0.4: bonus -= (total_turns / 10.0) ** 2 * 10.0
            ...
            if mtype in ("ATTACK", "CAPTURE"):
                ...
                for relay in w.my_planets:
                    ...
                    if aim1 and is_path_safe(w, src.id, src.x, src.y, aim1[0], bnow, relay.id, start_time=aim1[4]):
                        ...
                        if pos_t1:
                            ...
                            aim2 = w.aim(relay_f, tgt, s_relay_t1, max_wait=0)
                            if aim2 and is_path_safe(w, relay.id, relay_f.x, relay_f.y, aim2[0], s_relay_t1, tgt.id, start_time=t1):
                                if t1 + aim2[1] < total_turns and s_relay_t1 >= min_ships_to_own_at(w, tgt.id, t1 + aim2[1], extra) + w.total_keep(relay.id):
                                    if best_hop is None or (t1 + aim2[1]) < best_hop[0]:
                                        best_hop = (t1 + aim2[1], relay, aim1[0], bnow, t1, aim1[4])
                if best_hop:
                    raw_missions.append((src.id, best_hop[1].id, best_hop[2], best_hop[4], best_hop[3], profit, position_bonus(w, best_hop[1], phase), 0.0, 0.0, "STAGED_RELAY", best_hop[5], tgt.id, w.step + best_hop[4]))
                    continue
            
            raw_missions.append((src.id, tgt.id, angle, total_turns, req_cost, profit, bonus, ships_at_arrival, threat_total, mtype, wait_turns))
    ...
    for src in w.my_planets:
        ...
        if allow_rear_push and bud >= REAR_MIN_SHIPS and dist_to_front:
            ...
            if candidates:
                ...
                if aim_r and is_path_safe(w, src.id, src.x, src.y, aim_r[0], bud, relay.id, start_time=aim_r[4]):
                    raw_missions.append((src.id, relay.id, aim_r[0], aim_r[1], bud, max_profit_from[relay.id], position_bonus(w, relay, phase), w.timelines[relay.id]["ships_at"].get(aim_r[1], relay.ships), 0.0, "RELAY", aim_r[4]))
    ...
    m_p, s_p = get_stats([m[5] for m in raw_missions], 0.0, 20.0); m_t, s_t = get_stats([time_penalty(m[3] + m[10]*0.5) for m in raw_missions], 35.0, 15.0)
    ...
    for r in raw_missions:
        ...
        eff_t = time_penalty(trn + wait * 0.5); np = (prf - m_p) / s_p; nt = (eff_t - m_t) / s_t; nb = (bon - m_b) / s_b; nc = (abs(shp - cst) - m_c) / s_c; nth = (thrt - m_th) / s_th
...
def phase_overcrowded_push(w: WorldModel, spent: dict, moves: list, planned_commitments: list):
    locks = active_locks(w.step)
    for src in w.my_planets:
        if src.id in locks or src.ships <= 100: continue
        bud = w.attack_budget(src.id, spent)
        if bud < 1: continue
        best_tgt, min_dist = None, float('inf')
        spd = fleet_speed(bud)
        for tgt in w.my_planets:
            if tgt.id == src.id: continue
            d = dist(src.x, src.y, tgt.x, tgt.y)
            if d / spd <= 5.0:
                if d < min_dist: min_dist = d; best_tgt = tgt
        if best_tgt:
            aim_res = w.aim(src, best_tgt, bud)
            if aim_res:
                ang, total_t, _, _, wait = aim_res
                if is_path_safe(w, src.id, src.x, src.y, ang, bud, best_tgt.id, start_time=wait):
                    # logger.info(f"[OVERCROWDED] Turn {w.step}: {src.id} -> {best_tgt.id} | ships={bud}, dist={min_dist:.1f}")
                    moves.append([src.id, float(ang), bud]); spent[src.id] = spent.get(src.id, 0) + bud; planned_commitments.append((total_t, w.player, best_tgt.id, bud))
...
def phase_comet_evac(w: WorldModel, spent: dict, moves: list, planned_commitments: list):
    ...
    for src in w.my_planets:
        ...
        if 0 < life < COMET_EVAC_TURNS:
            ...
            aim_res = w.aim(src, tgt, bud)
            if aim_res:
                ang_r, total_r, _, _, w_r = aim_res
                if is_path_safe(w, src.id, src.x, src.y, ang_r, bud, tgt.id, start_time=w_r):
                    # logger.info(f"[EVAC] Turn {w.step}: Comet {src.id} -> {tgt.id} (EVAC) | ships={bud}, life={life}")
                    moves.append([src.id, float(ang_r), bud]); spent[src.id] = spent.get(src.id, 0) + bud; planned_commitments.append((total_r, w.player, tgt.id, bud))
...
def phase_tactical_evac(w: WorldModel, spent: dict, moves: list, planned_commitments: list):
    ...
    for src in w.my_planets:
        ...
        if len(enemies_targeting) >= 2 and soon_arrival:
            ...
            target_list = safe_others if safe_others else others; tgt = min(target_list, key=lambda p: dist(src.x, src.y, p.x, p.y))
            aim_res = w.aim(src, tgt, bud)
            if aim_res:
                ang_r, total_r, _, _, w_r = aim_res
                if is_path_safe(w, src.id, src.x, src.y, ang_r, bud, tgt.id, start_time=w_r):
...
def update_params(step, initial_by_id, planets):
    ...
    if globals().get("TUNE_MODE", False):
        ...
        return
    if is_1v1:
        if is_early: ALPHA, BETA, GAMMA, DELTA, EPSILON, MAX_ARRIVAL_TURNS = 1.54, 2.04, 1.56, 1.69, 4.51, 21
        else: ALPHA, BETA, GAMMA, DELTA, EPSILON, MAX_ARRIVAL_TURNS = 1.61, 4.74, 1.69, 4.24, 2.17, 28
    else:
        if is_early:
            ALPHA, BETA, GAMMA, DELTA, EPSILON, MAX_ARRIVAL_TURNS = 0.52, 2.77, 1.77, 2.12, 1.30, 26
        else:
            ALPHA, BETA, GAMMA, DELTA, EPSILON, MAX_ARRIVAL_TURNS = 2.36, 1.56, 1.52, 5.75, 3.07, 34
...
def agent(obs, config=None):
    ...
    if (time.perf_counter() - t0) < DEADLINE_SOFT:
        phase_overcrowded_push(w, spent, moves, planned_commitments)
        phase_comet_evac(w, spent, moves, planned_commitments); phase_tactical_evac(w, spent, moves, planned_commitments)
    final_moves = finalize(moves, w); t_elapsed = time.perf_counter() - t0; 
