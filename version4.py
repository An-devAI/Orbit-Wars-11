...
class WorldModel:
    ...
    def aim(self, src, tgt, ships):
        ...
        margin = 0.6 if tgt.id in self.comet_ids else 0.85
...
def phase7_hub_and_spoke(w: WorldModel, spent: dict, moves: list, planned_commitments: list):
    if len(w.my_planets) < 2: return
    
    frontier = w.enemy_planets if w.enemy_planets else w.neutral_planets
    if not frontier: return
    
    # 1. Bầu HUB linh hoạt: Chọn hành tinh an toàn nhất sát tiền tuyến
    def dist_to_f(p): return min(dist(p.x, p.y, t.x, t.y) for t in frontier)
    
    # Ưu tiên các hành tinh an toàn và không phải là sao chổi
    safe_planets = [p for p in w.my_planets if w.holds_full(p.id) and p.id not in w.comet_ids]
    if not safe_planets: 
        # Rơi vào trường hợp xấu nhất: chọn hành tinh không phải sao chổi
        safe_planets = [p for p in w.my_planets if p.id not in w.comet_ids]
    if not safe_planets: 
        # Nếu tất cả đều là sao chổi
        safe_planets = w.my_planets
        
    hub = min(safe_planets, key=dist_to_f)
    dist_hub_to_front = dist_to_f(hub)
    ...
    for src in w.my_planets:
        ...
        # Xử lý sao chổi: Nếu sao chổi sắp hết hạn, luôn luôn rút toàn bộ quân
        if src.id in w.comet_ids:
            life = comet_life(src.id, w.comet_map)
            if life < COMET_EVAC_TURNS:
                send = int(src.ships) - spent.get(src.id, 0)
                is_evac = True
        
        # Nếu không phải trường hợp di tản khẩn cấp, áp dụng logic dồn quân thông thường
        if not is_evac:
            d_src = dist_to_f(src)
            # Chỉ dồn quân nếu src xa tiền tuyến hơn HUB ít nhất 25% hoặc 10 đơn vị khoảng cách
            if d_src < max(dist_hub_to_front * REAR_DIST_RATIO, dist_hub_to_front + 10): continue
            
            bud = w.attack_budget(src.id, spent)
            if bud < REAR_MIN_SHIPS: continue
            send = int(bud * HUB_SEND_RATIO)
            
        if send < 1: continue
        
        aim_r = w.aim(src, hub, send)
        if not aim_r: continue
        angle, turns, _, _ = aim_r
        
        # Cho phép di chuyển tối đa REAR_MAX_TURNS, luôn cần đường đi an toàn
        if turns > REAR_MAX_TURNS or not is_path_safe(w, src, angle, send, hub.id): continue
        
        tag = "(EVAC)" if is_evac else "(HUB)"
        print(f"[PUSH] Turn {w.step}: {src.id} -> {hub.id} {tag} | ships={send}, turns={turns}")
        moves.append([src.id, float(angle), send])
        spent[src.id] = spent.get(src.id, 0) + send
        planned_commitments.append((turns, w.player, hub.id, send))
...
def agent(obs, config=None):
    ...
    if (time.perf_counter() - t0) < DEADLINE_SOFT and not w.is_late and len(w.my_planets) >= 2:
        phase7_hub_and_spoke(w, spent, moves, planned_commitments)
